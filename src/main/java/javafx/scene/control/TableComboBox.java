 ...
 * @since JavaFX 2.1
 */
public class TableComboBox<T> extends ComboBoxBase<T> {
    ...
    private ObjectProperty<StringConverter<T>> converter =
            new SimpleObjectProperty<>(this, "converter", defaultStringConverter());
     ...
     */
    public TableComboBox(ObservableList<T> items) {
        getStyleClass().add(DEFAULT_STYLE_CLASS);
        setAccessibleRole(AccessibleRole.COMBO_BOX);
        setItems(items);
        setSelectionModel(new TableComboBoxSelectionModel<>(this));
...
//    }
    ...
    // package for testing
    static class TableComboBoxSelectionModel<T> extends SingleSelectionModel<T> {

        private final TableComboBox<T> comboBox;
        ...
        public TableComboBoxSelectionModel(final TableComboBox<T> cb) {
            if (cb == null) {
